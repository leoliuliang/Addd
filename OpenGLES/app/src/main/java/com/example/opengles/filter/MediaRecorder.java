...
//    编码封装格式  h264
    ...
    public void start(float speed) throws IOException {
        ...
        //开启编码
        mMediaCodec.start();
        //重点 ，需要编码gpu里面的数据画面  ， 肯定要调用 opengl 函数，函数只能运行在opengl线程里
        //創建OpenGL 环境
    ...
    //编码   textureId数据  并且编码,  主动拿取openglfbo数据
    public void fireFrame(final int textureId, final long timestamp) {
        ...
        mHandler.post(new Runnable() {
            public void run() {
                //opengl环境已经创建好在handlerThread里，这里就可以直接调用了，调用draw，就可以将fbo数据textureId传给surface
